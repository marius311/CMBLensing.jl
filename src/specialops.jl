...
ud_grade(L::FullDiagOp{<:Field{Fourier}}, θnew) = FullDiagOp(getbasis(L.f)(ud_grade((L.unsafe_invert ? nan2zero.(L.f) : L.f),θnew,mode=:fourier,deconv_pixwin=false,anti_aliasing=false)))
