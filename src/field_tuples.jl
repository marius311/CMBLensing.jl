...
struct Field2Tuple{F1<:Field,F2<:Field,B1,B2} <: Field{Pix,Spin,Basis2Tuple{B1,B2}}
    f1 :: F1
    f2 :: F2
    Field2Tuple(f1::F1,f2::F2) where {P1,P2,S1,S2,B1,B2,F1<:Field{P1,S1,B1},F2<:Field{P2,S2,B2}} = new{F1,F2,B1,B2}(f1,f2)
    Field2Tuple{F1,F2,B1,B2}(f1::F1,f2::F2) where {P1,P2,S1,S2,B1,B2,F1<:Field{P1,S1,B1},F2<:Field{P2,S2,B2}} = new{F1,F2,B1,B2}(f1,f2)
    # would be nice if this could look like...
...
# vector conversion
@generated function getindex(f::F2T{F1,F2},::Colon) where {F1,F2}
    :(vcat($((:(f.f1.$x[:]) for x in fieldnames(F1))...),
           $((:(f.f2.$x[:]) for x in fieldnames(F2))...)))
end
...
# dot product
dot(a::F2T{F1,F2},b::F2T{F1,F2}) where {F1,F2} = dot(a[1],b[1])+dot(a[2],b[2])

# transpose multiplication (avoid storing temporary array)
Ac_mul_B(a::F2T,b::F2T) = (x=a.f1'*b.f1; x+=a.f2'*b.f2)

# basically allows ϕ*f where ϕ::FlatS0Map and f::F which is used in LenseFlow
...
@swappable *(a::Field,b::F2T) = a.*b #todo: fix ambiguity for *(::F2T,::F2T)

# for simulating
white_noise(f::Type{F2T{F1,F2}}) where {F1,F2} = F2T(white_noise(F1),white_noise(F2))
